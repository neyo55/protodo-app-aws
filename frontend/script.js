// script.js - ProTodo v1.4 (S3 & Placeholder Fixes)
const API_BASE = '/api'; 
let todos = [];
let editModeId = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('todo-list')) {
        if (localStorage.getItem('token')) {
            ...
            if(!document.getElementById('login-email')) window.location.href = '/login'; 
        }
    }
});
...
async function handleLogin() {
    ...
    const data = await res.json();
    if(res.ok) { localStorage.setItem('token', data.token); localStorage.setItem('user', JSON.stringify(data.user)); window.location.href = '/app'; }
...
async function handleForgot() {
    ...
    if(res.ok) { localStorage.setItem('reset_email', email); window.location.href = '/reset-password'; }
...
async function handleReset() {
    ...
    if(res.ok) { showToast("Success!", "success"); window.location.href = '/login'; }
